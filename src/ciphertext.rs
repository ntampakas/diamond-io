use crate::{operations::bit_decompose, poly_add, poly_sub, PublicKey};
...
impl Ciphertext {
    ...
    pub fn add_gate(&self, pub_key: &PublicKey, idx_a: usize, idx_b: usize) -> Vec<Vec<Vec<u64>>> {
        let params = pub_key.params();
        let ring = params.ring();
        let m = *params.m();
    ...
    pub fn mul_gate(&self, pub_key: &PublicKey, idx_a: usize, idx_b: usize) -> Vec<Vec<Vec<u64>>> {
        ...
        let tau = bit_decompose(params, &minus_b_idx_a);
