    ...
    S: PolyHashSampler<T, P, M>,
{
    ...
    for (idx, input) in input.iter().enumerate() {
        let m = if *input {
            &obfuscation.m_preimages[idx].1
        } else {
            &obfuscation.m_preimages[idx].0
        };
        let q = matrix_op
            .mul(&ps[idx], &m)
            .map_err(|e| ObfuscationError::MatrixError(e.to_string()))?;
        let n = if *input {
            &obfuscation.n_preimages[idx].1
        } else {
            &obfuscation.n_preimages[idx].0
        };
        let p = matrix_op
            .mul(&q, &n)
            .map_err(|e| ObfuscationError::MatrixError(e.to_string()))?;
        let k = if *input {
            &obfuscation.k_preimages[idx].1
        } else {
            &obfuscation.k_preimages[idx].0
        };
        let v = matrix_op
            .mul(&q, &k)
            .map_err(|e| ObfuscationError::MatrixError(e.to_string()))?;
